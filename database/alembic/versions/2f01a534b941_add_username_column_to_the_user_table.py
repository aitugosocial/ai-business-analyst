"""add username column to the user table

Revision ID: 2f01a534b941
Revises: fe5967720e5b
Create Date: 2026-07-30 17:17:35.710423
...
revision: str = '2f01a534b941'
down_revision: Union[str, Sequence[str], None] = 'fe5967720e5b'
...
def upgrade() -> None:
    ...
    # Deduplicate any duplicate usernames by suffixing user ID before enforcing unique index
    op.execute("""
        UPDATE users u1
        SET username = u1.username || '_' || u1.id
        FROM users u2
        WHERE u1.username = u2.username AND u1.id > u2.id AND u1.username IS NOT NULL;
    """)
